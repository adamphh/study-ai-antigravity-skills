...
# Slash Command: /index_refresh

Workflow này tự động thực thi làm mới bộ chỉ mục mã nguồn 2 Tầng.

## Cú pháp sử dụng trong Chat:
- `/index_refresh` hoặc `/index_refresh all`: Làm mới toàn bộ cả Tầng 1 (Shared Core) và Tầng 2 (Project hiện tại).
- `/index_refresh core`: Chỉ làm mới Tầng 1 (Shared Core Index dùng chung).
- `/index_refresh project`: Chỉ làm mới Tầng 2 (Project Local Index dự án hiện tại).
